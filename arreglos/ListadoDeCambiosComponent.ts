...
import { Estado } from '../../../../core/interfaces/liberaciones.interfaces';
...
export class ListadoDeCambiosComponent implements OnInit{
  ...
  errorMessage: string | null = null;    // error general (pantalla principal)
  ...
  onChange() {
    ...
    this.getChangeServices.get_arg_estados('pase').subscribe((estados) => {
      ...
      this.getChangeServices.postCRQ(body).subscribe({
        next: (result) => {
          // ✅ Filtrar por PENDING Y por estados válidos de Argos
    ...
    }); // ← Cierre del subscribe de get_arg_estados
...
//        for (let i = 0; i < result.length; i++) {
//          if (result[i].varJfrogVar == 'PENDING') {
//            if(this.estadosArgos.length === 0 || this.estadosArgos.includes(result[i].varEstado)){ //editado filtro
//              this.listaDeCRM.push(result[i]);
//            }
//
//          }
    ...
    this.getChangeServices
      ...
      .subscribe({
        ...
        error: (error: any) => {
          ...
          this.messageService.error(msg); // opcional, toast
        // NO cierro el modal para que vea el error y pueda corregir
        },
      });
